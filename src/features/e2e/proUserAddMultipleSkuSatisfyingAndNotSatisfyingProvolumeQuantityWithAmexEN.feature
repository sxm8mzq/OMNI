Feature: Pro User Place Order Of Multiple STH Items With Amex In English
  ...
  Scenario Outline: Pro user should be able to add multiple sth items and place order with amex
    ...
    Then the user click on "My Account"
    Then the user waits for text "Edit Addresses" to be visible
    ...
    Then the user clicks on the "hd-logo-pro-en" icon at index "1"
    Then the user presses keyboard key "Escape"
    Then the user enters text "<skuProVolumen>" in textbox with placeholder "What can we help you find?"
    Then the user clicks on the "search" icon at index "1"
    Then the user waits for text "Add To Cart" to be visible
    Then the user validates if "We will call you within 2 business days of placing the order to schedule the delivery." text is not visible
    Then the user enters value "<noOfProduct2>" in dropdown "Choose Your Quantity"
    Then the user validates if "We will call you within 2 business days of placing the order to schedule the delivery." text is visible
    Then the user add product to cart
    Then the user click on "View Cart"
    Then the user waits for text "My Cart: " to be visible
    ...
    Examples:
      | email                  | password | firstName | lastName    | skuProVolumen | noOfProduct2 | address            | city        | province | phoneNum   | postalCode |
      | pro_user_003@gmail.com | Test@123 | Test      | WebdriverIO | 1001187769    | 3            | 428 Ellesmere Road | Scarborough | Ontario  | 4704243224 | M1R 4E6    |