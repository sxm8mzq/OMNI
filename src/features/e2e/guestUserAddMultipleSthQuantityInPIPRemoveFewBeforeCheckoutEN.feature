Feature: Guest User Add Multiple STH Quantity In PIP And Remove Few Before Checkout In English

  Background: User launches url
    Given the user launches the url

  @e2e @regressionTest @guestUser @amex @sth @mCycle
  Scenario Outline: Guest user should able to add multiple quantity in pip and remove few before checkout in english
    ...
    Examples:
      | firstName | lastName | address            | city        | province | postalCode | phoneNum   | increasedNoOfProduct | decreasedNoOfProduct |
      | Laila     | Belanger | 428 Ellesmere Road | Scarborough | Ontario  | M1R 4E6    | 4569089080 | 8                    | 5                    |