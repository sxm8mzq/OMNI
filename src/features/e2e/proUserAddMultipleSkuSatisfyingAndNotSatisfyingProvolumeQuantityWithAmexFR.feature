Feature: Pro User Place Order Of Multiple STH Items With Amex In French

  Background: User launches url
    Given the user launches the url

  @e2e @regressionTest @sth @amex @mCycle @proUser @registeredUser
  Scenario Outline: Pro user should be able to add multiple sth items and place order with amex
    ...
    Then the user click on "Mon compte"
    Then the user waits for text "Modifier l’adresse" to be visible
    ...
    Then the user clicks on the "hd-logo-pro-fr" icon at index "1"
    Then the user presses keyboard key "Escape"
    Then the user enters text "<skuProVolumen>" in textbox with placeholder "Que cherchez-vous?"
    Then the user clicks on the "search" icon at index "1"
    Then the user waits for text "Ajouter au panier" to be visible
    Then the user validates if "Nous vous contacterons dans les 2 jours ouvrables de votre commande pour fixer la date et l’heure de la livraison." text is not visible
    Then the user enters value "<noOfProduct>" in dropdown "Choisissez la quantité"
    Then the user validates if "Nous vous contacterons dans les 2 jours ouvrables de votre commande pour fixer la date et l’heure de la livraison." text is visible
    Then the user add product to cart
    Then the user click on "Afficher le panier"
    Then the user waits for text "Mon Panier: " to be visible
    Then the user click on "Passer à la caisse"
    Then the user waits for text " Sommaire du panier: " to be visible
    Then the user validates if "Tarification au volume " text is visible
    ...
    Then the user validates if " LIVRAISON AU VOLUME PRO: " text is visible

    Examples:
      | email                  | password | firstName | lastName    | skuProVolumen | noOfProduct | address            | city        | province | phoneNum   | postalCode |
      | pro_user_004@gmail.com | Test@123 | Test      | WebdriverIO | 1001187769    | 3           | 428 Ellesmere Road | Scarborough | Ontario  | 4944243224 | M1R 4E6    |