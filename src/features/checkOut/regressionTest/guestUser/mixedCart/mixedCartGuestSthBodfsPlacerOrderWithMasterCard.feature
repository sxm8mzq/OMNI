Feature: Verify Guest User should be able to place mixed cart order with MasterCard

  Background: User launches url
    Given the user launches the url

  @checkOut @mCycle @regressionTest @guestUser @mixedCart @masterCard
  Scenario Outline: Guest User should be able to place mixed cart order with MasterCard
    ...
    Examples:
      | sth        | bodfs      | firstName | lastName | address            | city        | province | postalCode | phone      |
      | 1000740986 | 1000741513 | Test      | User     | 480 ellesmere road | Scarborough | Ontario  | M1R4E6     | 4165985212 |