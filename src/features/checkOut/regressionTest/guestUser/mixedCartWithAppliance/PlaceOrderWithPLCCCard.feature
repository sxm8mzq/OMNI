Feature: Verify place order with PLCC card

  Background: User launches url
    Given the user launches the url

  @checkOut @mCycle @regressionTest @guestUser @mixedCartWithAppliance @plcc @cswe
  Scenario Outline: Guest user should able to verify place order with PLCC card
    ...
    Then the user waits for webelement "acl-image__image" with html attribute "class" to be visible
    Then the user clicks on the webelement with html tag "id" as "fulfillment_DELIVERY_SCD_<bodfs>"
    ...
    Then the user verifies if the webelement with html attribute "class" as "acl-weight--bold acl-color--title" at index "2" has text "Order Number:"

    Examples:
      | bodfs      | boss       | appliance  | firstName | lastName | address            | city        | province | postalCode | phone      |
      | 1000741513 | 1000746500 | 1001075809 | Olivia    | Beaulieu | 428 Ellesmere Road | Scarborough | Ontario  | M1R 4E6    | 4704243224 |