Feature: Verify place order with hd commercial card

  Background: User launches url
    Given the user launches the url

  @checkOut @mCycle @regressionTest @guestUser @mixedCartWithAppliance @hdcc @cswe
  Scenario Outline: User should able to verify place order with hd commercial card
    ...
    Then the user enters text "<BodfsRegOpc>" in textbox with placeholder "What can we help you find?"
    ...
    Then the user enters text "<SthFreeOpc>" in textbox with placeholder "What can we help you find?"
    Then the user clicks on the webelement with html tag "class" as "acl-action-button"
    Then the user waits for webelement "hdca-product" with html attribute "class" to be visible
    Then the user add product to cart
    Then the user click on "Continue Shopping"
    ...
    Then the user click on "Continue Shopping"
    Then the user clicks on the webelement with html tag "type" as "search"
    Then the user enters text "<BossRegOpc>" in textbox with placeholder "What can we help you find?"
    Then the user clicks on the webelement with html tag "class" as "acl-action-button"
    Then the user waits for webelement "hdca-product" with html attribute "class" to be visible
    Then the user add product to cart
    Then the user click on "Continue Shopping"
    Then the user clicks on the webelement with html tag "type" as "search"
    Then the user enters text "<ApplianceRegOPC>" in textbox with placeholder "What can we help you find?"
    Then the user clicks on the webelement with html tag "class" as "acl-action-button"
    Then the user waits for webelement "hdca-product" with html attribute "class" to be visible
    Then the user add product to cart
    ...
    Then the user clicks on the webelement with html tag "id" as "fulfillment_BOPIS_<BopisFreeOpc>"
    Then the user waits for webelement "acl-image__image" with html attribute "class" to be visible
    Then the user clicks on the webelement with html tag "id" as "fulfillment_SHIP_TO_HOME_<SthFreeOpc>"
    Then the user waits for webelement "acl-image__image" with html attribute "class" to be visible
    Then the user clicks on the webelement with html tag "id" as "fulfillment_BOSS_<BossRegOpc>"
    Then the user waits for webelement "acl-image__image" with html attribute "class" to be visible
    Then the user clicks on the webelement with html tag "id" as "fulfillment_DELIVERY_SCD_<BodfsRegOpc>"
    ...
    Examples:
      | BodfsRegOpc | SthFreeOpc | BopisFreeOpc | BossRegOpc | ApplianceRegOPC | firstName | lastName | address          | city        | province | postalCode | phone      |
      | 1000741513  | 1001226868 | 1001127436   | 1000746500 | 1001075809      | Olivia    | Beaulieu | 428 ELLESMERE RD | Scarborough | Ontario  | M1R 4E6    | 4704243224 |