Feature: Verify guest pick up in store details in OPC and order confirmation page

  Background: User launches url
    Given the user launches the url

  @checkOut @mCycle @regressionTest @guestUser @bopis @visaCard @cswe
  Scenario Outline: Guest User should able to verify pick up in store details in OPC and order confirmation page
    ...
    Then the user validates if webelement with html attribute "symbol" as "location" is visible
    Then the user validates if " When would you like to pick up? " text is visible
    Then the user validates if "STANDARD SHIPPING" text is not visible
    ...
    Examples:
      ...
      | TestUser  | WebdriverIO | 4567891234 |