Feature: Registered User STH Place Order With Amex
  # Home page new Registered user place STH order with Amex

  Background: User launches url
    Given the user launches the url

  @checkOut @mCycle @regressionTest @homePageNewRegisteredUser @registeredUser @sth @amex
  Scenario Outline: Registered user should able to place order with amex
    ...
    Examples:
      | postalCode | firstName | lastName | password | address            | city        | province | phoneNum   |
      | M1R 4E6    | Johnny    | King     | Test@123 | 428 Ellesmere road | Scarborough | Ontario  | 4123456789 |